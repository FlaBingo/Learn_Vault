...
export default function MarkdownFormatter({content, isExpanded, contentWrapperRef}: {content: string, isExpanded: boolean, contentWrapperRef: Ref<HTMLDivElement>}) {
  return (
    <div
    ...
    >
      <ReactMarkdown
        ...
        components={{
          ...
          // Bold: Darker text in light mode, White text in dark mode
          strong: ({ node, ...props }) => (
            <strong
          ...
          // Blockquotes: Light gray border/text in light mode, Dark gray border/text in dark mode
          ...
          // Links: Blue in light, Lighter Blue in dark for contrast
          a: ({ node, ...props }) => (
            <a
          ...
          pre: ({ node, ...props }) => (
            <pre
              // bg-gray-50 for light mode, bg-gray-900 (or zinc-900) for dark mode
            ...
            return (
              <code
                className={`font-mono text-xs ${
                  isBlock
                    ? "block" // pre handles padding/bg for blocks
                    : "bg-gray-100 dark:bg-gray-800 rounded px-1 py-0.5" // Inline code style
                } ${className || ""}`}
                {...props}
              >
                {children}
